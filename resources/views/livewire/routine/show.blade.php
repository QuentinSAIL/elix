<div>
    <div class="md:hidden bg-custom-accent flex flex-col">
        <div class="flex-1 p-4 flex flex-col"
            ...
            <h2 class="title-color text-center mb-4 font-semibold">
                @if ($isFinished)
            ...
            @if ($currentTaskIndex === null && !$isFinished)
                <div class="bg-custom p-4 rounded-lg shadow mb-4">
                    <div class="flex items-center justify-between mb-3">
                        <h3 class="text-base font-bold title-color">{{ $routine->name }}</h3>
                        <span class="bg-elix/20 text-elix py-0.5 px-2 rounded-full text-xs">
                        ...
                        @if ($routine->completed_count > 0)
                            ...
                            @if ($routine->last_completed_at)
                                <div class="flex items-center gap-2">
                                    ...
                                    <span>{{ __('Last completed') }}: {{ $routine->last_completed_at->diffForHumans() }}</span>
                                </div>
                            @endif
                        @endif
                    </div>
            ...
            @if ($isFinished)
                <div class="bg-custom p-4 rounded-lg shadow mb-4 text-center">
                    <div class="flex flex-col items-center space-y-3">
                        <div class="text-elix">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                 fill="currentColor" class="w-20 h-20 animate-bounce" aria-hidden="true">
                        ...
                        <h3 class="text-xl font-bold title-color">{{ __('Congratulations!') }}</h3>
                        <p class="text-base">{{ __('You have successfully completed your routine') }}</p>

                        <div class="grid grid-cols-2 gap-3 w-full">
                            <div class="bg-elix/10 p-3 rounded-lg">
                                <div class="text-xs uppercase tracking-widest">{{ __('Total Time') }}</div>
                                <div class="font-mono text-xl font-bold text-elix">
                                    {{ gmdate('H:i:s', $routine->tasks->sum('duration')) }}
                                </div>
                            </div>
                            <div class="bg-elix/10 p-3 rounded-lg">
                                <div class="text-xs uppercase tracking-widest">{{ __('Tasks Completed') }}</div>
                                <div class="font-mono text-xl font-bold text-elix">
                                    {{ $routine->tasks->count() }}
                                </div>
                            </div>
            ...
            @if ($currentTaskIndex !== null && !$isFinished)
                    ...
                    <div class="relative w-64 h-64 mx-auto">
                        <svg viewBox="0 0 100 100" class="absolute inset-0 w-full h-full" aria-hidden="true">
                            <circle cx="50" cy="50" r="45" :stroke-dasharray="circum"
                                    :stroke-dashoffset="circum - (percent() / 100) * circum"
                        ...
                        <div class="absolute inset-0 flex flex-col items-center justify-center space-y-2">
                            <div class="text-center">
                                <div class="text-[10px] uppercase tracking-widest">{{ __('Elapsed Time') }}</div>
                                <div class="font-mono text-3xl font-bold text-elix" x-text="hhmmss(elapsedAllMs())"></div>
                            </div>
                            <div class="text-center">
                                <div class="text-[10px] uppercase tracking-widest">{{ __('Remaining (Current Task)') }}</div>
                                <div class="font-mono text-xl font-semibold title-color" x-text="hhmmss(remainingMs)"></div>
                            </div>
                            <div class="text-center">
                                <div class="text-[10px] uppercase tracking-widest">{{ __('Total Remaining') }}</div>
                                <div class="font-mono text-xl font-semibold title-color" x-text="hhmmss(totalRemainingMs())"></div>
                            </div>
                            <div class="text-center">
                                <div class="text-[10px] uppercase tracking-widest">{{ __('Progress') }}</div>
                                <div class="text-base font-semibold text-elix"
                ...
                <div class="sticky bottom-0 left-0 right-0 bg-custom-accent/80 backdrop-blur p-3 flex justify-center gap-2">
...
@endif
            ...
            <div class="mt-4 flex-1 overflow-y-auto">
                <div class="space-y-3">
                        ...
                        <div id="{{ $task->id }}" wire:key="m-task-{{ $task->id }}"
                            ...
                            <div class="flex items-start gap-3">
                                <div class="flex-1">
                                        ...
                                        @if ($currentTaskIndex === null && !$isFinished)
                                            <div class="ml-auto flex items-center">
                                                <livewire:routine-task.form :routine="$routine" :task="$task"
                                                    wire:key="m-task-form-{{ $task->id }}" mobile />
                                                <button type="button" wire:click="deleteTask('{{ $task->id }}')"
                                                        wire:key="m-delete-task-{{ $task->id }}"
                                                        aria-label="{{ __('Delete task') }}" class="ml-2">
                                                    <flux:icon.trash class="cursor-pointer" variant="micro" aria-hidden="true" />
                                                </button>
                                                <button type="button" wire:click="duplicateTask('{{ $task->id }}')"
                                                        wire:key="m-duplicate-task-{{ $task->id }}"
                                                        aria-label="{{ __('Duplicate task') }}" class="ml-2">
                                                    <flux:icon.document-duplicate class="cursor-pointer" variant="micro" aria-hidden="true" />
                                                </button>
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    @if ($task->description)
                                        <div class="mt-1 text-sm flex items-start gap-2">
                                            <flux:icon.flag class="text-elix mt-0.5" aria-hidden="true" />
                                            <span>@limit($task->description, 120)</span>
                                        </div>
                                    @endif

                                    <div class="mt-2 flex items-center">
                                        <div class="flex items-center">
                                            <flux:icon.clock aria-hidden="true" />
                                            <span class="ml-2 text-sm">{{ gmdate('H:i:s', $task->duration) }}</span>
                                        </div>
                    ...
                    @if ($currentTaskIndex === null && !$isFinished)
                        <div class="w-full bg-custom-accent hover:bg-custom-accent/70 p-4 rounded-md border-2 border-dashed flex items-center justify-center text-center cursor-pointer transition-all"
                                aria-label="{{ __('Add new task') }}">
                            <livewire:routine-task.form :routine="$routine" wire:key="m-task-form-create" mobile />
                        </div>
                    @endif
                </div>
            </div>
    ...
    <div class="hidden md:grid bg-custom-accent grid-cols-2 h-[64vh]">
            ...
            @if ($isFinished)
                <div class="bg-custom p-6 rounded-lg shadow-md mb-6 text-center">
                    <div class="flex flex-col items-center justify-center space-y-4">
                        <div class="celebration-icon text-elix mb-2">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                 fill="currentColor" class="w-24 h-24 animate-bounce" aria-hidden="true">
                                <path fill-rule="evenodd"
                                      d="M8.603 3.799A4.49 4.49 0 0112 2.25c1.357 0 2.573.6 3.397 1.549a4.49 4.49 0 013.498 1.307 4.491 4.491 0 011.307 3.497A4.49 4.49 0 0121.75 12a4.49 4.49 0 01-1.549 3.397 4.491 4.491 0 01-1.307 3.497 4.491 4.491 0 01-3.497 1.307A4.49 4.49 0 0112 21.75a4.49 4.49 0 01-3.397-1.549 4.49 4.49 0 01-3.498-1.306 4.491 4.491 0 01-1.307-3.498A4.49 4.49 0 012.25 12c0-1.357.6-2.573 1.549-3.397a4.49 4.49 0 011.307-3.497 4.49 4.49 0 013.497-1.307zm7.007 6.387a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z"
                                      clip-rule="evenodd" />
                            </svg>
                        ...
                        <div class="stats grid grid-cols-2 gap-4 w-full mt-2">
...
@endunless
            ...
            <div class="col-span-8 flex flex-col" wire:ignore.self
                 x-data="{
                    ...
                    initSortable() {
                        ...
                        this.sortable = new Sortable(this.$refs.list, {
                            ...
                            animation: 150,
                            onEnd: evt => {
                                this.tasks.splice(evt.newIndex, 0, this.tasks.splice(evt.oldIndex, 1)[0])
                                        ...
                                        <div class="ml-auto my-auto flex items-center">
                                            @if ($currentTaskIndex === null && !$isFinished)
                                                <button type="button" class="drag-handle cursor-move" aria-label="{{ __('Reorder task') }}">
                                                    <flux:icon.bars-4 aria-hidden="true" />
                                                </button>
                                            @endif
                                        </div>
                                    </div>
